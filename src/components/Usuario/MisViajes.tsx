...
import { getAsync } from '../../api/dataManager'; // Quitamos patch y del
...
export const MisViajes = () => {
  ...
  useEffect(() => {
    if (userId) {
      ...
      if (userJson) {
        ...
        setUsuario(user);
        const aprobado = user?.estadoConductor?.toLowerCase() === 'aprobado' || 
  ...
  const cargarDatos = async (idUsuario: number) => {
    ...
    try {
      ...
      const resPub = await getAsync<any>(`viaje/mis-publicaciones/${idUsuario}`);
      if (resPub.data && resPub.data.data) setViajesPublicados(resPub.data.data);
  ...
  const bufferToBase64 = (buffer: any) => {
    if (!buffer?.data) return 'https://ui-avatars.com/api/?name=User&background=random';
  ...
  // Filtros Pasajero
  const proximosPasajero = solicitudes.filter(s => 
    s.estadoSolicitud?.toLowerCase() === 'aprobada' && (s.viaje?.viajeEstado?.toLowerCase() === 'disponible' || s.viaje?.viajeEstado?.toLowerCase() === 'pendiente')
  );
  const recientesPasajero = solicitudes.filter(s => 
    s.estadoSolicitud?.toLowerCase() === 'pendiente' || s.estadoSolicitud?.toLowerCase() === 'denegada'
  );

  // Filtros Conductor
  const proximosConductor = viajesPublicados.filter(v => v.viajeEstado?.toLowerCase() === 'disponible' || v.viajeEstado?.toLowerCase() === 'pendiente');
  const realizadosConductor = viajesPublicados.filter(v => v.viajeEstado?.toLowerCase() !== 'disponible' && v.viajeEstado?.toLowerCase() !== 'pendiente');
    ...
    <div className="pb-5" style={{ backgroundColor: '#ffffff', minHeight: '100vh' }}>
      ...
      <div className="container pt-4 pb-3">
        <div className="d-flex justify-content-between align-items-center">
          <h2 className="fw-bold m-0" style={{ color: colorTextoGrisOscuro, fontSize: '1.5rem' }}>
            {vistaActiva === 'pasajero' ? 'Mis próximos viajes' : 'Próximos viajes'}
          </h2>
          
          {isConductorAprobado && (
            <button 
              className="btn bg-white position-relative"
              style={{ border: '1px solid #ced4da', borderRadius: '8px', padding: '6px 16px', fontWeight: '600', color: colorTextoGrisOscuro }}
              onClick={() => setVistaActiva(vistaActiva === 'pasajero' ? 'conductor' : 'pasajero')}
            >
              Mis viajes
              <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill" style={{ backgroundColor: colorNaranja, border: '2px solid white' }}>
      ...
      {vistaActiva === 'pasajero' && (
        <>
          <div style={{ backgroundColor: bgVerdeClaro }} className="py-4">
            <div className="container">
              {proximosPasajero.length === 0 ? (
                <p className="text-muted text-center my-4">No tenés próximos viajes confirmados.</p>
              ) : (
              ...
              <div className="text-center mt-3">
                <span className="text-decoration-underline fw-bold" style={{ color: '#1f5c2f', cursor: 'pointer' }}>
          ...
          <div className="container py-4">
            <h3 className="fw-bold mb-4" style={{ color: colorTextoGrisOscuro, fontSize: '1.3rem' }}>Mis solicitudes recientes</h3>
            {recientesPasajero.length === 0 ? (
              <p className="text-muted text-center my-4">No hay solicitudes recientes.</p>
            ) : (
      ...
      {vistaActiva === 'conductor' && (
        <>
          <div style={{ backgroundColor: bgVerdeClaro }} className="py-4">
            <div className="container">
              {proximosConductor.length === 0 ? (
                <p className="text-muted text-center my-4">No tenés viajes publicados activos.</p>
              ) : (
          ...
          <div style={{ boxShadow: '0 -4px 10px rgba(0,0,0,0.03)' }} className="py-4">
            <div className="container">
              <h3 className="fw-bold mb-4" style={{ color: colorTextoGrisOscuro, fontSize: '1.3rem' }}>Viajes realizados</h3>
              {realizadosConductor.length === 0 ? (
                <p className="text-muted text-center my-4">Aún no realizaste ningún viaje.</p>
              ) : (
...
const TarjetaPasajeroProximo = ({ solicitud, hora, foto, onCancelar }: any) => {
  ...
  return (
    <div className="card border-0 mb-3" style={{ borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
        ...
        <div className="row">
          <div className="col-8 col-md-7">
            ...
            <div className="mt-3 d-flex align-items-center">
              ...
              <span className="fw-bold text-dark" style={{ fontSize: '0.95rem' }}>{viaje?.usuarioConductor?.nombreUsuario} {viaje?.usuarioConductor?.apellidoUsuario}</span>
            </div>
            <p className="text-muted m-0 mt-1" style={{ fontSize: '0.85rem' }}>{viaje?.vehiculo?.marca} {viaje?.vehiculo?.modelo} - {viaje?.vehiculo?.color} - {viaje?.vehiculo?.patente}</p>
          </div>
          ...
          <div className="col-4 col-md-5 d-flex flex-column align-items-end justify-content-start">
            <div className="text-muted d-flex align-items-center mb-1" style={{ fontSize: '0.9rem' }}>
              <i className="bi bi-calendar3 me-2"></i> <span>{new Date(viaje?.viajeFecha).toLocaleDateString('es-AR')}</span>
            </div>
      ...
      <div className="text-end" style={{ marginTop: '-20px', marginRight: '15px', position: 'relative', zIndex: 2 }}>
        <button 
          ...
          style={{ border: '1px solid #dc3545', color: '#dc3545', fontSize: '0.85rem', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}
        >
          Cancelar solicitud
        </button>
      </div>
...
const TarjetaPasajeroReciente = ({ solicitud, hora, foto, onCancelar }: any) => {
  ...
  return (
    <div className="card bg-white mb-3" style={{ borderRadius: '16px', border: '1px solid #eaeaea', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
      <div className="card-body p-4 position-relative">
        <div className="row align-items-center">
          ...
          <div className="col-6 col-md-5">
            ...
            <div className="mt-3 d-flex align-items-center">
              ...
              <span className="fw-bold text-dark" style={{ fontSize: '0.9rem' }}>{viaje?.usuarioConductor?.nombreUsuario}</span>
            </div>
            <p className="text-muted m-0 mt-1" style={{ fontSize: '0.8rem' }}>{viaje?.vehiculo?.marca} {viaje?.vehiculo?.modelo} - {viaje?.vehiculo?.patente}</p>
          </div>
          ...
          <div className="col-3 col-md-4 d-flex flex-column justify-content-center">
            <div className="text-muted d-flex align-items-center mb-1" style={{ fontSize: '0.85rem' }}>
              <i className="bi bi-calendar3 me-2"></i> {new Date(viaje?.viajeFecha).toLocaleDateString('es-AR')}
            </div>
...
const TarjetaConductorActivo = ({ viaje, hora, onCancelar, onComenzar }: any) => {
  const isCompleto = viaje.solicitudesAprobadas >= viaje.viajeCantLugares;

  return (
    <div className="mb-4">
      <div className="card border-0" style={{ borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <div className="card-body p-0">
          ...
          <div 
            ...
            style={{ borderTop: '1px solid #eaeaea', backgroundColor: '#ffffff', cursor: 'pointer' }}
            onClick={() => alert("En construcción: Ver pasajeros y solicitudes")}
          >
            <span className="fw-bold" style={{ color: '#0d6efd', fontSize: '0.95rem' }}>Ver pasajeros y solicitudes</span>
          </div>
        </div>
      </div>
      ...
      <div className="d-flex mt-3 gap-3">
        <button 
          ...
          style={{ border: '2px solid #dc3545', color: '#dc3545', fontSize: '0.95rem', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}
        >
          Cancelar Viaje
        </button>
        <button 
          onClick={onComenzar}
          className="btn bg-white w-50 rounded-pill fw-bold py-2" 
          style={{ border: '2px solid #0dcaf0', color: '#0d6efd', fontSize: '0.95rem', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}
        >
          Comenzar viaje
        </button>
      </div>
...
const TarjetaConductorRealizado = ({ viaje, hora }: any) => {
  return (
    <div className="card bg-white mb-3" style={{ borderRadius: '16px', border: '1px solid #eaeaea' }}>
      <div className="card-body p-0">
        <div className="row p-4 align-items-center">
        ...
        >
          <span className="fw-bold text-dark" style={{ fontSize: '0.95rem' }}>Ver pasajeros</span>
        </div>
      </div>
    </div>
  );
