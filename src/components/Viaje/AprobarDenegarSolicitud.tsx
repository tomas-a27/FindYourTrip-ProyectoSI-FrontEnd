...
export const AprobarDenegarSolicitud = () => {
  ...
  const [solicitudPendiente, setSolicitudPendiente] = useState<
    SolicitudViajeDTO[]
  >([]);
  const [solicitudPendienteLoading, setSolicitudPendienteLoading] =
    useState(true);
  const [solicitudPendienteError, setSolicitudPendienteError] = useState(false);

  const [solicitudAprobadaRechazada, setSolicitudAprobadaRechazada] = useState<
    SolicitudViajeDTO[]
  >([]);
  const [
    solicitudAprobadaRechazadaLoading,
    setSolicitudAprobadaRechazadaLoading,
  ] = useState(true);
  const [solicitudAprobadaRechazadaError, setSolicitudAprobadaRechazadaError] =
    useState(false);
  ...
  const cargarDatos = async (esRecargaOculta = false) => {
    ...
    try {
      const resPendiente = await getAsync<any>(
    ...
    try {
      const resAprobadas = await getAsync<any>(
        'viaje/solicitudes-aprobadas-rechazadas-viaje/' +
          location.state.viajeId,
      );
      setSolicitudAprobadaRechazada(
        resAprobadas.data?.data || resAprobadas.data || [],
      );
    ...
    try {
      const resViaje = await getAsync<any>(
        'viaje/detalle/' + location.state.viajeId,
      );
  ...
  return (
    ...
    >
        ...
        >
          <div className="card-body p-4">
            <h5 className="fw-bold mb-2 text-dark">
              {location.state.viajeOrigen}{' '}
              <i className="bi bi-arrow-right mx-2 text-muted"></i>{' '}
              {location.state.viajeDestino}
            </h5>
            <div
              className="d-flex flex-wrap text-muted gap-4 mt-2"
              style={{ fontSize: '0.95rem' }}
            >
              <span>
                <i className="bi bi-calendar3 me-2"></i>{' '}
                {location.state.viajeFecha.split('-').reverse().join('/') ||
                  location.state.viajeFecha}
              </span>
              <span className="fw-bold" style={{ color: '#2d4a2d' }}>
                <i className="bi bi-person-check-fill me-2"></i>
                Quedan:{' '}
                {viajeInfo?.lugaresDisponibles ??
                  location.state.lugaresDisponibles}{' '}
                lugares disponibles
              </span>
            </div>
          </div>
                          ...
                          <div className="d-flex justify-content-between align-items-start">
                            <div>
                              <div className="d-flex align-items-center mb-1">
                                ...
                                >
                                  {solicitud.usuario?.nombreUsuario?.charAt(0)}
      ...
      <div>
        <div
          className="container mt-5 mb-5 pb-5 text-center"
