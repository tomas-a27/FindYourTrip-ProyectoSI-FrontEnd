...
const ModalComenzarFinalizarViaje = ({
  ...
  const fechaHoraViaje =
    viajeFecha && viajeHorario
      ? new Date(`${viajeFecha}T${viajeHorario}`)
      : null;
  const hanPasadoMasDe24Horas =
    isComenzar &&
    !!fechaHoraViaje &&
    !Number.isNaN(fechaHoraViaje.getTime()) &&
    Date.now() - fechaHoraViaje.getTime() > 24 * 60 * 60 * 1000;
    ...
    <>
      {isComenzar ? (
        <button
          ...
          disabled={hanPasadoMasDe24Horas}
          style={{
            backgroundColor: hanPasadoMasDe24Horas ? '#e9ecef' : '#ffffff',
            border: hanPasadoMasDe24Horas
              ? '2px solid #adb5bd'
              : '2px solid #1f5c2f',
            color: hanPasadoMasDe24Horas ? '#6c757d' : '#1f5c2f',
            fontSize: '0.95rem',
            transition: 'all 0.2s ease',
            cursor: hanPasadoMasDe24Horas ? 'not-allowed' : 'pointer',
          }}
          onMouseEnter={(e) => {
            if (hanPasadoMasDe24Horas) return;
        ...
        createPortal(
          <div className="modal-overlay">
            <div className="custom-modal p-4 text-center">
              ...
              {errorMsg && (
                <div
                  className="alert alert-danger fw-bold fs-6 mb-4 p-2 d-flex align-items-center justify-content-center"
                  role="alert"
                >
